...
// --- PLANTILLA HTML (V8 - Rediseño con elementos más grandes) ---
const generateHTML = (data) => {
  ...
  // Lógica para las miniaturas: Ahora solo 2
  let otherImages = [];
  if (imagesArray.length > 1) {
    // Agarramos 2 imágenes aleatorias de las restantes
    otherImages = [...imagesArray].slice(1).sort(() => 0.5 - Math.random()).slice(0, 2);
  }

  // Si faltan imágenes para llenar los 2 espacios, repetimos la principal
  ...
  <body class="bg-gray-900 m-0 p-0 flex justify-center">
    <div class="relative bg-black overflow-hidden flex flex-col justify-between" style="width: 1080px; height: 1920px;">
      ...
      <!-- Header: Logo y Datos del Auto (Más grandes) -->
      <div class="relative z-20 pt-32 px-12 flex flex-col items-center w-full text-center">
        <div class="mb-16">
          <img src="https://carfiable.mx/wp-content/uploads/2026/01/negativo-color.svg" class="h-24 object-contain" />
        </div>
      ...
      <!-- Footer: Precios, Miniaturas y WhatsApp -->
      <div class="relative z-20 w-full pb-24 flex flex-col items-center">
        
        <!-- Sección Precios (Más grandes) -->
        <div class="w-full flex justify-center items-center gap-12 mb-12 px-10">
          <div class="flex flex-col items-center leading-none">
            <span class="text-2xl text-gray-300 uppercase font-bold mb-2">Precio</span>
            <span class="text-7xl font-black text-white">${precio}</span>
          </div>
          
          <div class="w-[2px] h-24 bg-white/30"></div>
          
          <div class="flex flex-col items-center leading-none">
            <span class="text-2xl uppercase font-bold text-[#3865E9] mb-2">Enganche</span>
            <span class="text-8xl font-black text-white">${enganche}</span>
          </div>
        </div>

        <!-- Miniaturas: Ahora 2 Columnas -->
        <div class="w-full mb-16 px-4">
          <div class="flex h-64 w-full gap-4">
            ${otherImages.map(img => `
              <div class="flex-1 relative overflow-hidden rounded-xl border-2 border-white/10 shadow-2xl">
        ...
        <div class="w-full flex justify-center mb-8">
          <div class="flex items-center gap-6 text-white py-8 px-16 rounded-full shadow-[0_0_50px_rgba(56,101,233,0.4)]" style="background-color: #3865E9;">
            ...
            <span class="text-6xl font-black tracking-tighter">${data.telefono}</span>
          </div>
        </div>
        
        <p class="text-xl text-gray-400 font-medium opacity-80 text-center">*Aplican Restricciones.</p>
      </div>
    </div>
  </body>
  </html>
