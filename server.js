...
const generateHTML = (data) => {
  ...
  // Lógica de imágenes (Seguridad si vienen pocas fotos)
  const mainImage = data.exterior[0];
  let otherImages = [];
  if (data.exterior.length > 1) {
    otherImages = data.exterior.slice(1).sort(() => 0.5 - Math.random()).slice(0, 3);
  } else {
    // Fallback si solo hay 1 foto: repetimos la principal
    otherImages = [mainImage, mainImage, mainImage];
  }
  ...
  <body class="bg-gray-900 m-0 p-0 flex justify-center">
    ...
    <div class="relative bg-black overflow-hidden flex flex-col justify-between" style="width: 1080px; height: 1920px;">
      ...
      <div class="relative z-20 pt-60 px-12 flex flex-col items-center w-full text-center">
        <div class="mb-12">
          <img src="https://carfiable.mx/wp-content/uploads/2026/01/negativo-color.svg" class="h-16 object-contain drop-shadow-lg" />
        </div>
        <div class="w-full">
          <p class="text-4xl font-bold text-gray-300 leading-none drop-shadow-md">${data.anio}</p>
          <h1 class="text-7xl font-black text-white uppercase italic leading-tight my-2 drop-shadow-xl">${data.modelo}</h1>
          <p class="text-2xl font-bold uppercase tracking-widest drop-shadow-md text-[#3865E9]">${data.version}</p>
        </div>
      </div>
      ...
      <div class="relative z-20 w-full pb-60 flex flex-col items-center">
        ...
        <div class="w-full flex justify-center items-center gap-8 mb-8 px-4">
          <div class="flex flex-col items-end leading-tight">
            ...
            <span class="text-4xl font-bold text-white drop-shadow-md">${precio}</span>
          </div>
          <div class="w-1 h-16 bg-gray-600"></div>
          <div class="flex flex-col items-start leading-tight">
            <span class="text-xl uppercase font-bold text-[#3865E9]">Enganche</span>
            <span class="text-5xl font-black text-white drop-shadow-lg">${enganche}</span>
          </div>
        </div>
        ...
        <div class="w-full mb-12">
          <div class="flex h-40 w-[120%] -ml-[10%] gap-2 transform -skew-x-12">
            ${otherImages.map(img => `
              <div class="flex-1 relative overflow-hidden border-r border-white/20 shadow-lg bg-gray-800">
        ...
        <div class="w-full flex justify-center mb-4">
          <div class="flex items-center gap-4 text-white py-4 px-10 rounded-full shadow-lg" style="background-color: #3865E9; box-shadow: 0 10px 15px -3px rgba(56, 101, 233, 0.4);">
        ...
        <p class="text-xs text-gray-500 max-w-md leading-tight opacity-60 text-center">*Aplican Restricciones. Precios y condiciones sujetos a disponibilidad.</p>
      </div>
    </div>
  </body>
  </html>
  ...
  try {
    ...
    const browser = await puppeteer.launch({ 
      args: [
        ...
        '--disable-dev-shm-usage', // Clave para evitar crash en Docker
        '--disable-gpu'
      ],
    ...
    // Esperar a que cargue Tailwind y Fuentes (networkidle0)
    ...
    res.status(500).send('Error interno generando imagen');
  }
});
